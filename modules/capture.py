...
class CaptureThread(QThread):
    ...
    def __init__(self, video_source, video_config=None):
        ...
        self.pause = False
        self.cur_frame = None
        self.video_fps = None  # Store video FPS for external access

        self.framerate = Framerate()
        
        # Add debugging info
        print(f"CaptureThread initialized with source: {video_source}")
    ...
    def _read_stream(self):
        ...
        # Try different backends for different cameras
        ...
        for backend in backends:
            ...
            if cap.isOpened():
                print(f"Successfully opened video source: {self.video_source} with backend {backend}")
                break
            else:
                cap.release()
        
        if cap is None or not cap.isOpened():
            print(f"ERROR: Could not open video source {self.video_source} with any backend")
            return
    ...
    def _read_video(self):
        ...
        # Don't apply video config for prerecorded videos to preserve original timing
        if self.video_config is not None:
            print("Warning: Video config applied to prerecorded video - this may affect timing!")
            self.video_config.set(cap)
        
        # grab the file's native FPS; fall back to 30 if we can't read it
        fps = cap.get(cv2.CAP_PROP_FPS) or 30.0
        self.video_fps = fps  # Store FPS for external access
        frame_interval = 1.0 / 15.0
        
        print(f"Video playback: Using native FPS {fps:.2f}, frame interval {frame_interval:.4f}s")
        ...
        while not self.stop_threads:
            ...
            if frame_count % 30 == 0:  # Print every 30 frames
                print(f"Frame {frame_count}: elapsed={elapsed:.4f}s, to_wait={to_wait:.4f}s")
            
            if to_wait > 0:
                time.sleep(to_wait)
            elif to_wait < -0.1:  # If we're significantly behind
                print(f"Warning: Frame processing took {elapsed:.4f}s (expected {frame_interval:.4f}s)")
    ...
    def run(self):
        ...
        print(f"CaptureThread.run() - Source: {self.video_source}")
        if is_image(self.video_source):
            print("Using _read_image() method")
            self._read_image()
        elif is_video(self.video_source):
            print("Using _read_video() method")
            self._read_video()
        else:
            print("Using _read_stream() method")
            self._read_stream()
