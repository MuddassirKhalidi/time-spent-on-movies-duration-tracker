...
class MXFace():
    ...
    def detect_faces_direct(self, image):
        """Direct face detection using OpenCV DNN as fallback for manual uploads"""
        # For now, use a simple fallback - return empty results
        # This will be improved later with proper face detection
        print("Using fallback detection - no faces detected")
        annotated_frame = AnnotatedFrame(image)
        annotated_frame.boxes = []
        annotated_frame.keypoints = []
        annotated_frame.scores = []
        return annotated_frame
