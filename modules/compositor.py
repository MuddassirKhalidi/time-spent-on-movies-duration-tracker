...
class Compositor(QObject):
    ...
    def set_search_targets(self, search_targets):
        """Set the search targets for highlighting found persons"""
    ...
    def draw_name(self, frame, obj):
        (left, top, right, bottom) = obj.bbox
        # Show "Found [name]" for search targets, otherwise just the name
        if obj.name and obj.name in self.search_targets:
            label = f'Found {obj.name}'
        else:
        ...
        # Use red color for search targets, blue for others (RGB format)
        text_color = (255, 0, 0) if obj.name and obj.name in self.search_targets else MxColors.Blue
    ...
    def draw_objects(self, frame, tracked_objects):
        for obj in tracked_objects:
            ...
            if self.bbox_checkbox.isChecked(): 
                line_thickness = int(self.line_thickness_slider.value())
                # Highlight with red overlay if this person is a search target
                if obj.name and obj.name in self.search_targets:
                    # Create red overlay with opacity
                    overlay = frame.copy()
                    cv2.rectangle(overlay, (left, top), (right, bottom), (255, 0, 0), -1)  # Fill rectangle with red (RGB format)
                    alpha = 0.5  # Opacity level (0.0 = transparent, 1.0 = opaque)
                    cv2.addWeighted(overlay, alpha, frame, 1 - alpha, 0, frame)
                    # Also draw border
                    cv2.rectangle(frame, (left, top), (right, bottom), (255, 0, 0), line_thickness)
                else:
