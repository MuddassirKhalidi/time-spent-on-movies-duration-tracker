...
class FaceDatabase:
    ...
    def load_specific_profile(self, database_path, profile_name):
        """Load only a specific profile from the database"""
        print(f'loading specific profile "{profile_name}" from "{database_path}"...', end='', flush=True)
        self.database = defaultdict(dict)
        self.database_path = Path(database_path)

        # Only load the specific profile directory
        profile_path = Path(database_path) / profile_name
        if profile_path.exists():
            for file in profile_path.iterdir():
                if file.suffix.lower() == '.embed':
                    embed = np.loadtxt(file)
                    self.database[profile_name][file.name] = embed
        print(f'Done.')
...
class DatabaseViewerWidget(QWidget):
    def __init__(self, face_database: FaceDatabase, photo_upload_callback=None, target_name=None):
        super().__init__()

        self.face_database = face_database
        self.db_path = face_database.database_path
        self.photo_upload_callback = photo_upload_callback  # Callback to trigger photo upload
        self.target_name = target_name  # Specific target name if provided
        ...
        self.add_button = QPushButton("Add Query")
    ...
    def load_profiles(self):
        ...
        # Only reload all profiles if no specific target is provided
        if not self.target_name:
            self.face_database.load_database_embeddings(self.db_path)

        # Determine which profiles to show
        if self.target_name:
            # Only show the specific target profile
            profiles_to_show = [self.target_name] if os.path.exists(os.path.join(self.db_path, self.target_name)) else []
        else:
            # Show all profiles
            profiles_to_show = [name for name in os.listdir(self.db_path) if os.path.isdir(os.path.join(self.db_path, name))]

        # Iterate over profiles to display
    ...
    def add_profile(self):
        profile_name, ok = QInputDialog.getText(self, 'Add Query', 'Enter person name to search for:')
        if ok:
            if profile_name:
                profile_path = os.path.join(self.db_path, profile_name)
