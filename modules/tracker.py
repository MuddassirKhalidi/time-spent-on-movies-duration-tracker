...
class FaceTracker(QObject):
    ...
    """
    def __init__(self, face_database: FaceDatabase, shared_mxface=None, camera_id=None, stream_id=None, video_fps=None):
        super().__init__()
        self.tracker = BYTETracker()
        self.camera_id = camera_id
        self.stream_id = stream_id
        self.video_fps = video_fps  # Store video FPS for timing control
        #self.mxface = MockMXFace(Path('assets/models'))
        if shared_mxface is not None:
            self.mxface = shared_mxface
            self._is_shared_mxface = True
        else:
            self.mxface = MXFace(Path('./models-img-search'))
            self._is_shared_mxface = False
    ...
    def stop(self):
        ...
        # Only stop MXFace if it's not shared (i.e., we created it)
        if not hasattr(self, '_is_shared_mxface'):
            self.mxface.stop()
...
class DetectionThread(QThread):
    ...
    def _update_detections(self):
        camera_id = getattr(self.face_tracker, 'camera_id', 'unknown')
        
        # Use video FPS for timeout if available, otherwise use default 30 FPS
        timeout = 0.033  # Default 30 FPS
        if self.face_tracker.video_fps is not None:
            timeout = 1.0 / self.face_tracker.video_fps
            print(f"DetectionThread using video FPS: {self.face_tracker.video_fps:.2f}, timeout: {timeout:.4f}s")
        
        try:
            annotated_frame = self.face_tracker.mxface.detect_get(timeout=timeout)
            self.face_tracker.current_frame = annotated_frame
            self.framerate.update()
            # Debug: Show which camera is processing frames
            ...
            if self._frame_count % 30 == 0:  # Every 30 frames
                print(f"Camera {camera_id}: Processed {self._frame_count} frames, current frame shape: {annotated_frame.image.shape}")
        except queue.Empty:
            return
